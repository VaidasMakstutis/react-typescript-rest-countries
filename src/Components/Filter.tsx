...
const Filter = ({ countries, areaRef, regionRef, activeButton, setActiveButton, setShowCountries }: IFilterProps) => {
  ...
  return (
    <div className="filter-options">
      ...
      <button ref={areaRef} id="filter-area"onClick={findCountriesSmallerThanLtu}>Countries are smaller than Lithuania</button>
      <button ref={regionRef} id="filter-region" onClick={findCountriesInOceania}>Countries are in Oceania</button>
    </div>
  );
};
